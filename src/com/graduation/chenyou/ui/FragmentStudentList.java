...
import com.graduation.chenyou.adapter.StudentAdapter;
...
public class FragmentStudentList extends Fragment implements OnClickListener {
	// 定义控件
	...
	private void getView2Init(View view) {
		lv_student_list = (ListView) view.findViewById(R.id.lv_student_list);
	}
	...
	private void setStudentListData(final Context context) {
		// 获取关注的用户
		String getStudentUrl = JsonToBeanUtils.url + "getTeacherStudent";
		Map<String, String> parameter = new HashMap<String, String>();
		parameter.put("tid", UserInfo.uid);
		// 网络请求
		VolleyRequest.RequestPost(getStudentUrl, "getTeacherStudent",
				parameter, new VolleyInterface() {
					...
					public void onSuccess(String result) {
						try {
							...
							// 设置数据设配器
							lv_student_list.setAdapter(new StudentAdapter(
									context, studentList));
//							// 设置关注的人列表item项点击事件
//							lv_student_list.setOnItemClickListener(new OnItemClickListener() {
//										@Override
